...
ssize_t senders::send_iam(int socket_fd, const string& seat, string& message)
{
    message = "IAM" + seat + DELIMETER;
    return common::write_to_socket(socket_fd, message.data(), message.length());
}

ssize_t senders::send_busy(int socket_fd, const string& seats, string& message)
{
    message = "BUSY" + seats + DELIMETER;
    return common::write_to_socket(socket_fd, message.data(), message.length());
}

ssize_t senders::send_deal(int socket_fd, int16_t deal_type, const string& start_seat, const vector<string>& cards, string& message)
{
...
ssize_t senders::send_trick(int socket_fd, int16_t trick_number, const vector<string>& cards, string& message)
{
...
ssize_t senders::send_score(int socket_fd, const map<string, int32_t>& scores, string& message)
{
    message = "SCORE";
...
ssize_t senders::send_total(int socket_fd, const map<string, int32_t>& scores, string& message)
{
    message = "TOTAL";
    for (const auto& score : scores) { message += score.first + std::to_string(score.second); }
    message += DELIMETER;
